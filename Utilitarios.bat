...
:: Define os caminhos exatos garantindo que não caia no System32 por causa do modo Admin
set "NOVO_ARQUIVO=%~dp0Atualizacao_Temp.bat"
set "ATUALIZADOR=%temp%\atualizador.bat"

powershell -NoProfile -ExecutionPolicy Bypass -Command "$ProgressPreference = 'SilentlyContinue'; [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; try { Invoke-WebRequest -Uri '%URL_SCRIPT%' -OutFile '%NOVO_ARQUIVO%' -UseBasicParsing } catch { exit 1 }"

if not exist "%NOVO_ARQUIVO%" (
    color 0C
...
:: Cria um mini-script na pasta TEMP para evitar bugs de permissao
echo @echo off > "%ATUALIZADOR%"
echo echo Aplicando a atualizacao... Aguarde. >> "%ATUALIZADOR%"
echo timeout /t 2 /nobreak ^>nul >> "%ATUALIZADOR%"
echo move /y "%NOVO_ARQUIVO%" "%~f0" >> "%ATUALIZADOR%"
echo start "" "%~f0" >> "%ATUALIZADOR%"
echo del "%%~f0" >> "%ATUALIZADOR%"

echo.
echo Atualizacao concluida! Reiniciando o programa para aplicar...
start "" "%ATUALIZADOR%"
exit
