...
:is_admin
call :verificar_atualizacao

:menu
cls
color 0B
echo.
echo                                                 BEM VINDO A VERSAO %VERSAO_ATUAL%
echo.
...
echo ===============================================================================
echo.
echo QUAIS TECLAS VOCE DESEJA ATIVAR O MACRO?
echo.
echo Teclas suportadas: Z, X, C, R, T, F, E
echo.
echo Digite as teclas juntas (Exemplo: zxr). 
echo Para ativar TODAS, apenas aperte ENTER.
set "teclas_macro="
set /p teclas_macro="Sua escolha: "
if "%teclas_macro%"=="" set "teclas_macro=zxcrtfe"
echo.
...
:: Insercao dinamica das teclas escolhidas (usando goto para manter formato seguro)
echo %teclas_macro% | findstr /i "r" >nul
if errorlevel 1 goto skip_mac_r
echo $R:: >> "%ahk_file%"
echo { >> "%ahk_file%"
...
echo %teclas_macro% | findstr /i "z" >nul
if errorlevel 1 goto skip_mac_z
echo $z:: >> "%ahk_file%"
echo { >> "%ahk_file%"
...
echo %teclas_macro% | findstr /i "x" >nul
if errorlevel 1 goto skip_mac_x
echo $x:: >> "%ahk_file%"
echo { >> "%ahk_file%"
...
echo %teclas_macro% | findstr /i "c" >nul
if errorlevel 1 goto skip_mac_c
echo $c:: >> "%ahk_file%"
echo { >> "%ahk_file%"
...
echo %teclas_macro% | findstr /i "t" >nul
if errorlevel 1 goto skip_mac_t
echo $t:: >> "%ahk_file%"
echo { >> "%ahk_file%"
...
echo %teclas_macro% | findstr /i "f" >nul
if errorlevel 1 goto skip_mac_f
echo $f:: >> "%ahk_file%"
echo { >> "%ahk_file%"
...
echo %teclas_macro% | findstr /i "e" >nul
if errorlevel 1 goto skip_mac_e
echo $e:: >> "%ahk_file%"
echo { >> "%ahk_file%"
...
echo                     Macro habilitado para as teclas: %teclas_macro%
echo ===============================================================================
pause
goto menu_macro
...
set "URL_SCRIPT=https://raw.githubusercontent.com/Gml1010/UPDVERSION/main/Utilitarios.exe"
set "URL_NOTAS=https://raw.githubusercontent.com/Gml1010/UPDVERSION/main/novidades.txt"

set "TEMP_VERSION=%temp%\versao_temp.txt"

:: Baixa o arquivo version.txt ja limpando espacos e quebras de linha invisiveis
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ProgressPreference = 'SilentlyContinue'; [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; try { $v = (Invoke-WebRequest -Uri '%URL_VERSAO%' -UseBasicParsing -TimeoutSec 5).Content.Trim(); Set-Content -Path '%TEMP_VERSION%' -Value $v -NoNewline -Encoding ASCII } catch { exit 1 }"
...
:: Le a versao baixada (agora 100%% limpa)
...
:: Se encontrar atualizacao (versao diferente):
color 0E
echo.
...
:: Usa a pasta TEMP para evitar bugs com acentos no caminho
set "NOVO_ARQUIVO_TEMP=%temp%\Atualizacao_Temp.exe"
set "ATUALIZADOR=%temp%\atualizador.bat"
set "NOTAS_TEMP=%temp%\novidades.txt"

:: Deleta lixos de atualizacoes anteriores se existirem
if exist "%NOVO_ARQUIVO_TEMP%" del /f /q "%NOVO_ARQUIVO_TEMP%" >nul 2>&1

:: Baixa o arquivo EXECUTAVEL novo para a pasta TEMP
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ProgressPreference = 'SilentlyContinue'; [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; try { Invoke-WebRequest -Uri '%URL_SCRIPT%' -OutFile '%NOVO_ARQUIVO_TEMP%' -UseBasicParsing } catch { exit 1 }"

:: Baixa o arquivo de texto de novidades (silenciosamente)
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ProgressPreference = 'SilentlyContinue'; [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; try { Invoke-WebRequest -Uri '%URL_NOTAS%' -OutFile '%NOTAS_TEMP%' -UseBasicParsing } catch { }"

if not exist "%NOVO_ARQUIVO_TEMP%" (
    color 0C
    echo.
    echo [ERRO] Falha ao baixar a nova versao do script do GitHub.
    echo DICA: Verifique se o arquivo no GitHub se chama EXATAMENTE "Utilitarios.exe"
    timeout /t 6 >nul
    goto :eof
)

:: Cria um mini-script na pasta TEMP para aplicar a substituicao garantindo a extensao .exe
echo @echo off > "%ATUALIZADOR%"
echo timeout /t 2 /nobreak ^>nul >> "%ATUALIZADOR%"
:: Garante que o arquivo final substitua a versao atual e vire um .exe
echo move /y "%NOVO_ARQUIVO_TEMP%" "%~dp0%~n0.exe" >> "%ATUALIZADOR%"
echo if exist "%NOTAS_TEMP%" start notepad "%NOTAS_TEMP%" >> "%ATUALIZADOR%"
echo start "" "%~dp0%~n0.exe" >> "%ATUALIZADOR%"
:: Se o arquivo original era um .bat, ele vai apagar o .bat velho
echo if /i not "%~x0"==".exe" del "%~f0" >> "%ATUALIZADOR%"
:: Apaga os scripts invisiveis
echo del "%temp%\invisivel.vbs" >> "%ATUALIZADOR%"
echo del "%%~f0" >> "%ATUALIZADOR%"

:: Cria o script VBS para rodar o atualizador de forma 100%% oculta
echo Set WshShell = CreateObject("WScript.Shell") > "%temp%\invisivel.vbs"
echo WshShell.Run chr(34) ^& "%ATUALIZADOR%" ^& chr(34), 0, False >> "%temp%\invisivel.vbs"

echo.
echo Atualizacao baixada! Aplicando em segundo plano e reiniciando...
timeout /t 2 >nul
start wscript "%temp%\invisivel.vbs"
exit
